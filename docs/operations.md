...
`tidal-algo-set-mark` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for set analysis with seeded input scenarios, deterministic summary checks, and documented operating limits.
...
The score is deterministic. The current threshold is 151. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 3.
